...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        out = (x_norm * gamma) + beta # scale by gamma and shift by beta
        ...
        cache = x, mean, variance, std, gamma, x_norm, shape, axis, beta, eps
...
def batchnorm_backward(dout, cache):
    ...
    x, mean, variance, x_norm, gamma, _, _, _, beta, eps = cache
    ###########################################################################
    ...
    dx_norm = dout * gamma
    dstd = -np.sum(dx_norm * (x-mean), axis=0) / (std**2)          # derivative w.t.r. std
    dvar = 0.5 * dstd / std                                     # derivative w.t.r. var
    dx1 = dx_norm / std + 2 * (x-mean) * dvar / len(dout)          # partial derivative w.t.r. dx
...
def batchnorm_backward_alt(dout, cache):
    ...
    _, _, _, std, gamma, x_hat, shape, axis, _, _ = cache # expand cache
        ...
        for f in range(w.shape[0]):
            ...
            for i in range(H_):
                for j in range(W_):
                    ...
                    patch = x_n[:, h_start:h_end, w_start:w_end]
                    result = np.sum(patch * filter_f)
                    result += bias_f
...
def conv_backward_naive(dout, cache):
    ...
    """
    x, x_padded, w, b, conv_param = cache
        ...
        for f in range(F):
            for i in range(H_):
                for j in range(W_):
                    ...
                    dx[n, :, h_start:h_end, w_start:w_end] += w[f] * dout[n, f, i, j]
    if pad > 0:
